    ...
    <div class="mt-8 px-6">
        <div class="bg-white shadow-lg rounded-lg p-6">
            <h2 class="text-2xl font-semibold text-[#4C4C4C] mb-6">
                Editar Ficha Médica Ocupacional
            </h2>

            <form action="{{ route('ocupacional.fichas.actualizar', $ficha->id) }}" method="POST" class="space-y-6">
                @csrf
                @method('PUT')

                {{-- Datos ocultos --}}
                <input type="hidden" name="paciente_id" value="{{ $ficha->paciente_id }}">
                <input type="hidden" name="historia_clinica_id" value="{{ $ficha->historia_clinica_id }}">
                <input type="hidden" name="ruta_medica_id" value="{{ $ficha->ruta_medica_id }}">
                    ...
                    <div class="flex flex-wrap gap-4">
                        ...
                        <label class="flex items-center space-x-2">
                            <input type="radio" name="tipo_evaluacion" value="{{ $valor }}"
                                @checked(strtolower($ficha->tipo_evaluacion) === $valor)
                ...
                {{-- Puestos y tiempo --}}
                <div class="flex flex-col space-y-4">
                    <x-input-clinico label="Puesto al que Postula" name="puesto_postula" value="{{ $ficha->puesto_postula }}" />
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                        <x-input-clinico label="Puesto Actual" name="puesto_actual" value="{{ $ficha->puesto_actual }}" />
                        <x-input-clinico label="Tiempo en Puesto Actual" name="tiempo_puesto_actual"
                            value="{{ $ficha->tiempo_puesto_actual }}" />
                    </div>
                </div>
                    ...
                    <div class="flex flex-wrap gap-4">
                        ...
                        <label class="flex items-center space-x-2">
                            <input type="radio" name="exploracion_procesados" value="{{ $valor }}"
                                @checked($ficha->exploracion_procesados === $valor)
                            class="text-[#9C1C2A] focus:ring-[#9C1C2A]">
                            <span class="text-[#4C4C4C]">{{ $texto }}</span>
                        </label>
                        @endforeach
