<x-clinico-layout>
    ...
    <div class="p-6 bg-white rounded-lg shadow-md max-w-6xl mx-auto mt-8">
        <h2 class="text-xl font-semibold text-gray-700 mb-6">Editar Empresa</h2>
        ...
        <form action="{{ route('empresa.update', $empresa->id) }}" method="POST" class="space-y-6">
            ...
            <div class="grid grid-cols-1 md:grid-cols-3 gap-4">
                <div class="md:col-span-2">
                    <x-input-clinico label="Nombre o Razón Social" name="nombre"
                        value="{{ old('nombre', $empresa->nombre) }}" required class="uppercase" />
                </div>
                <div class="md:col-span-1">
                    <x-input-clinico label="Nombre Abreviado" name="nombre_abreviado"
                        value="{{ old('nombre_abreviado', $empresa->nombre_abreviado) }}" required class="uppercase" />
                </div>
            </div>
            ...
            <div>
                <x-input-clinico label="Dirección" name="direccion"
                    value="{{ old('direccion', $empresa->direccion) }}" class="uppercase" />
            </div>

            {{-- RUC, Rubro, Teléfono y Email Empresa --}}
            <div class="grid grid-cols-1 md:grid-cols-4 gap-4">
                <x-input-clinico label="RUC" name="ruc" maxlength="11"
                    value="{{ old('ruc', $empresa->ruc) }}" required
                    oninput="this.value=this.value.replace(/[^0-9]/g,'')" />

                <x-input-clinico label="Rubro" name="rubro"
                    value="{{ old('rubro', $empresa->rubro) }}" class="uppercase" />
            ...
            <div class="grid grid-cols-1 md:grid-cols-3 gap-4 border-t border-gray-200 pt-6">
                <x-input-clinico label="Persona de Contacto" name="persona_contacto"
                    value="{{ old('persona_contacto', $empresa->persona_contacto) }}" class="uppercase" />

                <x-input-clinico label="Teléfono de Contacto" name="telefono_contacto"
            ...
            <div class="grid grid-cols-1 md:grid-cols-3 gap-4 border-t border-gray-200 pt-6">
                ...
                <div class="flex flex-col md:flex-row items-center gap-2 mb-4">
                    ...
                    <div class="relative">
                        <select
                            id="departamento"
                            data-old="{{ old('departamento', $empresa->departamento) }}"
                ...
                <div class="flex flex-col md:flex-row items-center gap-2 mb-4">
                    ...
                    <div class="relative">
                        <select
                            id="provincia"
                            data-old="{{ old('provincia', $empresa->provincia) }}"
                            class="border border-gray-300 focus:border-[#9C1C2A] focus:ring-[#9C1C2A] rounded-md h-[42px] w-[42px] cursor-pointer bg-white mt-2">
                            <option value="">▼</option>
                        </select>
                    </div>
                ...
                <div class="flex flex-col md:flex-row items-center gap-2 mb-4">
                    ...
                    <div class="relative">
                        <select
                            id="distrito"
                            data-old="{{ old('distrito', $empresa->distrito) }}"
                            class="border border-gray-300 focus:border-[#9C1C2A] focus:ring-[#9C1C2A] rounded-md h-[42px] w-[42px] cursor-pointer bg-white mt-2">
                            <option value="">▼</option>
                        </select>
                    </div>
